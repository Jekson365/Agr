...
import {
  createGreenhouseHarvestChemical,
  updateGreenhouseHarvestChemical,
...
type Props = {
  ...
  editingChemical: GreenhouseHarvestChemical | null;
  onClose: () => void;
  onSaved: (chemical: GreenhouseHarvestChemical, isNew: boolean) => void;
};

/** Records a chemical applied to a greenhouse harvest — its name, date, and cost. */
export function GreenhouseHarvestChemicalFormModal({
  open,
  greenhouseHarvestId,
  editingChemical,
  onClose,
  ...
  async function handleSubmit() {
    ...
    try {
      if (isEditing) {
        const updated: GreenhouseHarvestChemical = { ...editingChemical, name: name.trim(), date, cost };
        await updateGreenhouseHarvestChemical(updated.id, updated);
        onSaved(updated, false);
      } else {
        const created = await createGreenhouseHarvestChemical({ greenhouseHarvestId, name: name.trim(), date, cost });
